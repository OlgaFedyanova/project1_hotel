...
public class Guests {
    ...
    public static HashMap<Integer, List<String>> guests = new HashMap<>(); //TODO почему здесь статик?
    //даты бронирования
    ...
    public static void guestsList() { //TODO: название getGuestList
    ...
    public static Integer createNewUserID(HashMap<Integer, List<String>> guestsStorage) {
        //TODO как проверяете что такой гость уже есть в базе?
        Integer counter = null; //TODO что происходит с counter и почему он изначально равен null?
    ...
    //поиск гостя по id
    public static HashMap<Integer, List<String>> searchGuest(int id) { //TODO: как вариант хэшмапа с гостями могла подаваться на вхож
        if (guests.containsKey(id)){
            System.out.println("Под этим номером нашелся Гость:\n" + guests.get(id));
        }
        else {
            System.out.println("Такого номера нет в базе!"); //TODO как еще мы можем это обработать?
        }
        return guests;
    }
    ...
    //изменение ФИО гостя
    //TODO: можно было заменить все методы ниже на один, в который на вход подается хэшмапа готовая имеющихся гостей
    // и те данные которые передает юзер через сканнер - в самом методе уже меняем то что изменил юзер (то что пришло на вход)
...
//TODO: id в хэшмапе можно было инкрементировать наполняя ее или в цикле пройти по каждому элементу и задать ему айдишник отдельно
        Guests guests = new Guests();
int id=0;
        //наполнение базы данных
        guests.addGuest(id++, new ArrayList<String>(Arrays.asList("Рыбаков Игорь Владимирович", "28", "мужчина")));
        guests.addGuest(id++, new ArrayList<String>(Arrays.asList("Сибгатуллов Шамиль Раильевич", "64", "мужчина")));
